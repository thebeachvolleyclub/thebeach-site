...
export default function BookingWidget() {
  ...
  return <div className="grid w-full min-w-0 max-w-full grid-cols-[minmax(0,1fr)] gap-0.5 overflow-hidden lg:grid-cols-[minmax(0,1.25fr)_minmax(0,0.75fr)]">
    <div className="min-w-0 overflow-hidden border border-black/10 bg-white p-5 text-black sm:p-7 lg:p-10">
      <div className="mb-7 flex flex-col items-stretch gap-4 border-b border-black/10 pb-5 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-black/40">Boka online</p><h3 className="mt-2 font-display text-3xl">400 kr <span className="block sm:inline">/ bana</span></h3></div><button type="button" onClick={() => { setShowMine(!showMine); if (!showMine && profile) loadMine(); }} className="w-full cursor-pointer border border-black px-4 py-3 text-[11px] font-bold uppercase tracking-[0.08em] hover:bg-black hover:text-lime sm:w-auto sm:shrink-0">{showMine ? "Boka tid" : "Mina bokningar"}</button></div>
    ...
    <div className="min-w-0 overflow-hidden border border-black/10 bg-cream p-5 text-black sm:p-7 lg:p-10"><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-black/40">4. Konto och betalning</p>{selected ? <><h3 className="mt-3 break-words font-display text-2xl">{selected.courtName}</h3><p className="mt-2 break-words text-sm text-black/55">{date} · {selected.startTime}–{selected.endTime} · {selected.durationMin} min</p></> : <p className="mt-3 text-sm text-black/45">Välj en ledig tid och bana för att fortsätta.</p>}
      ...
      {error ? <div role="alert" className="mt-4 border border-orange/25 bg-orange/10 p-4 text-sm font-semibold text-orange"><p>{error}</p>{error.toLowerCase().includes("profil") ? <Link href="/konto" className="mt-3 inline-flex text-xs font-bold uppercase tracking-[0.08em] underline underline-offset-4">Kontrollera Swish-numret →</Link> : null}</div> : null}<button type="button" disabled={!selected || submitting || !profileReady} onClick={checkout} className="mt-6 min-h-13 w-full cursor-pointer bg-black px-6 py-4 text-xs font-bold uppercase tracking-[0.08em] text-lime disabled:cursor-not-allowed disabled:opacity-35">{submitting ? "Godkänn betalningen i Swish…" : "Boka och betala 400 kr"}</button><p className="mt-3 text-center text-[11px] leading-relaxed text-black/45">Tiden hålls i 10 minuter. Betalningsbegäran skickas till Swish-numret i din profil.</p>
    </div>
  </div>;
}
