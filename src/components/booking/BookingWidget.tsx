...
import { bookingStreamLabel, hasRequiredStreamConsent } from "@/lib/bookingStreams";
...
export default function BookingWidget() {
  ...
  const [streamConsentAttested, setStreamConsentAttested] = useState(false);
  ...
  const loadSlots = useCallback(async (selectedVenue: string, selectedDate: string) => {
    setLoading(true); setError(""); setSelectedTime(""); setSelected(null); setStreamRequested(false); setStreamConsentAttested(false);
    try {
  ...
  const checkout = async () => {
    ...
    if (!hasRequiredStreamConsent(streamRequested, streamConsentAttested)) {
      setError("Bekräfta att alla deltagare godkänner filmningen.");
      return;
    }
    setSubmitting(true); setError("");
    try {
      const result = await api<{ bookingId: string }>("/api/booking/checkout", { method: "POST", body: JSON.stringify({ venueId, courtId: selected.courtId, date, startTime: selected.startTime, streamRequested, streamConsentAttested, clientReference: `web-${Date.now()}-${Math.random().toString(36).slice(2, 10)}` }) });
  ...
  return <div className="grid w-full min-w-0 max-w-full grid-cols-[minmax(0,1fr)] gap-0.5 overflow-hidden lg:grid-cols-[minmax(0,1.25fr)_minmax(0,0.75fr)]">
    ...
    <div className="min-w-0 overflow-hidden border border-black/10 bg-cream p-5 text-black sm:p-7 lg:p-10"><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-black/40">4. Konto och betalning</p>{selected ? <><h3 className="mt-3 break-words font-display text-2xl">{selected.courtName}</h3><p className="mt-2 break-words text-sm text-black/55">{date} · {selected.startTime}–{selected.endTime} · {selected.durationMin} min</p><p className="mt-2 text-lg font-bold">{selected.priceSek} kr</p></> : <p className="mt-3 text-sm text-black/45">Välj en ledig tid och bana för att fortsätta.</p>}
      ...
      {selected?.cameraEnabled && profileReady ? <><label className="mt-4 flex cursor-pointer items-start gap-3 border border-black/10 bg-white p-4"><input type="checkbox" checked={streamRequested} onChange={(event) => { setStreamRequested(event.target.checked); if (!event.target.checked) setStreamConsentAttested(false); }} className="mt-0.5 h-5 w-5 accent-black" /><span className="text-sm"><strong className="flex items-center gap-2"><svg viewBox="0 0 24 24" aria-hidden="true" className="h-4 w-4 fill-none stroke-current" strokeWidth="2"><rect x="3" y="6" width="13" height="12" rx="2" /><path d="m16 10 5-3v10l-5-3" /></svg>Beställ BeachTV-stream</strong><span className="mt-1 block text-black/45">Livestream från banans kamera.</span></span></label>{streamRequested ? <label className="mt-2 flex cursor-pointer items-start gap-3 border border-black/10 bg-white p-4"><input type="checkbox" checked={streamConsentAttested} onChange={(event) => setStreamConsentAttested(event.target.checked)} className="mt-0.5 h-5 w-5 accent-black" /><span className="text-sm font-semibold">Jag bekräftar att alla deltagare känner till och godkänner filmningen</span></label> : null}</> : null}
      {error ? <div role="alert" className="mt-4 border border-orange/25 bg-orange/10 p-4 text-sm font-semibold text-orange"><p>{error}</p>{error.toLowerCase().includes("profil") ? <Link href="/konto" className="mt-3 inline-flex text-xs font-bold uppercase tracking-[0.08em] underline underline-offset-4">Kontrollera Swish-numret →</Link> : null}</div> : null}<button type="button" disabled={!selected || submitting || !profileReady || !hasRequiredStreamConsent(streamRequested, streamConsentAttested)} onClick={checkout} className="mt-6 min-h-13 w-full cursor-pointer bg-black px-6 py-4 text-xs font-bold uppercase tracking-[0.08em] text-lime disabled:cursor-not-allowed disabled:opacity-35">{submitting ? "Godkänn betalningen i Swish…" : selected ? `Boka och betala ${selected.priceSek} kr` : "Välj tid och bana"}</button><p className="mt-3 text-center text-[11px] leading-relaxed text-black/45">Tiden hålls i 10 minuter. Betalningsbegäran skickas till Swish-numret i din profil.</p><p className="mt-2 text-center text-[11px] leading-relaxed text-black/45">Avboka inom en timme för full återbetalning. Därefter återbetalas beloppet minus 20 kr fram till 24 timmar före start.</p>
    </div>
  </div>;
}
