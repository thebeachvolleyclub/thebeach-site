    ...
    <section id="planlosning" className="bg-white px-5 py-16 sm:px-8 lg:px-14 lg:py-24">
      <div className="mx-auto max-w-6xl">
        <Reveal>
          ...
          <p className="mb-10 max-w-2xl text-sm leading-relaxed text-black/60">
            10 sandbanor inomhus och 7 utomhus, bar och kök i mitten, omklädning och toaletter intill entrén.
            Skalenlig ritning — använd den för att planera scen, sittning och stationer.
          </p>
        </Reveal>
        ...
        <ul className="mt-8 grid gap-x-8 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
          {YTOR.map((y) => (
            <li key={y.key} className="border-t border-black/10 pt-3">
              <span className="block text-sm font-semibold text-black">{y.namn}</span>
              <span className="block text-xs text-black/50">{y.beskrivning}</span>
            </li>
          ))}
        </ul>
      </div>
