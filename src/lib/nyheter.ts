...
import {
  fetchResultArticles,
  type Article,
} from "./resultArticles.core.ts";

export { parseResultArticle } from "./resultArticles.core.ts";
export type { Article, Block } from "./resultArticles.core.ts";
...
async function resultArticles(fresh = false): Promise<Article[]> {
  // The same immutable image runs in production and demo. Never resolve the
  // environment or fetch articles while Next is prerendering that image.
  await connection();
  try {
    ...
    return await fetchResultArticles(url, environment, fresh);
  } catch {
    return [];
  }
}
